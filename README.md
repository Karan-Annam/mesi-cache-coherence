...
4. **Atomics + sync primitives**: CAS and fetch-and-add as bus-locked RMW, then
   TAS lock, ticket lock, seqlock, hazard pointers, Treiber stack — driven by
...
(M-state writeback on a snooped read) and the BusUpgr→BusRdX race, which is the
one everybody gets wrong the first time.
